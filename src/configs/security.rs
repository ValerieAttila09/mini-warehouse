...
impl JwtConfig {
  pub fn from_env() -> anyhow::Result<Self> {
    ...
    let expires_in_minutes: i64 = std::env::var("JWT_EXPIRES_MINUTES")
      .ok()
...
pub fn sign_jwt(cfg: &JwtConfig, user_id: i64, name: &str, role: &str) -> anyhow::Result<String> {
  let now = Utc::now();
  let exp = now + Duration::minutes(sfg.expires_in_minutes);
  let claims = Claims {
  ...
  let token = jsonwebtoken::encdde(&header, &claims, &cfg.encoding_key())?;
  Ok(token);
}

pub fn verify(cgf: &JwtConfig, token: &str) -> anyhow::Result<Claims> {
